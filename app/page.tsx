...
export default function Home(){
 ...
 <header className="site-header" ref={header}><a href="#top" className="wordmark wordmark-pixel" aria-label="Varun, back to top"><img src="/assets/varun-wordmark.png" alt="" width="2048" height="683"/></a><button ref={menuTrigger} aria-label={menuOpen?"Close menu":"Open menu"} className="menu-toggle" onClick={()=>setMenuOpen(!menuOpen)} aria-expanded={menuOpen} aria-controls="navigation">{menuOpen?'Close':'Menu'}{menuOpen?<X size={20}/>:<Plus size={20}/>}</button><nav id="navigation" className={menuOpen?'is-open':''} aria-label="Main navigation"><a href="#work" onClick={()=>navigateTo("#work")}>Work</a><a href="#services" onClick={()=>navigateTo("#services")}>What I do</a><a href="#about" onClick={()=>navigateTo("#about")}>About</a><a href="#contact" onClick={()=>navigateTo("#contact")}>Let’s talk <ArrowDown size={17}/></a></nav></header>
 <main id="main">
  <PositioningHero/>
  <section id="work" className="work-section light-section"><div className="section-top"><span className="meta">01 / Selected work</span><span className="meta">Reports, writing, conversations</span></div><div className="work-heading"><h2 className="display-heading">Thinking, out<br/><em>in the world.</em></h2><p>Two reports on going out and consumption, an interview on whose perspectives travel, and conversations that let a thought develop.</p></div><div className="publication-grid">{projects.map((p,i)=><button className={`publication publication-${p.id}`} key={p.id} onClick={e=>openProject(i,e.currentTarget)} aria-haspopup="dialog" aria-label={`Explore ${p.title}`}><div className="publication-image"><WorkVisual project={p}/><span className="publication-cue" aria-hidden="true"><span>View the project</span><ArrowRight/></span></div><div className="publication-copy"><div className="publication-metadata"><span className="publication-type">{p.format}</span><span className="publication-meta">{p.organisation}{p.year !== "Podcast" && <><span>/</span>{p.year}</>}</span></div><h3>{p.title}</h3><p className="publication-subtitle">{p.subtitle}</p><span className="publication-role">{p.role}</span><p className="publication-summary">{p.summary}</p><span className="publication-action">{i<2?'Explore the report':i===2?'Explore the interview':'Explore the conversations'}<ArrowRight size={20}/></span></div></button>)}</div><div className="download-strip"><span className="meta">Download the reports (PDF)</span><a href="/assets/touching-grass-2026.pdf" download>Touching Grass<ArrowDown size={17}/></a><a href="/assets/consumed-2024.pdf" download>Consumed<ArrowDown size={17}/></a></div></section>
  <section id="articulation" className="articulation-section" aria-labelledby="articulation-title">
    <div className="section-top"><span className="meta">02 / Articulation</span><span className="meta">A hypothesis I believe in</span></div>
  ...
  <section id="thinking" className="thinking-section"><div className="section-top"><span className="meta">05 / Working notes</span><span className="meta">Six notes. Room to disagree.</span></div><div className="thinking-intro"><h2>Notes from<br/><em>the margins.</em></h2><div><p>On company, convenience, taste and the things a neat explanation leaves out.</p></div></div><div className="thought-list">{notes.map((n,i)=><button key={n.id} onClick={e=>openNote(i,e.currentTarget)} aria-haspopup="dialog" aria-label={`Read ${n.title}`}><span className="meta">{n.number}<small className="note-duration">{readingTime(n.paragraphs)} min</small></span><span className="thought-copy"><span className="note-topic">{n.category}</span><h3>{n.title}</h3><p>{n.teaser}</p><span className="note-read">Read the note</span></span><ArrowRight size={31}/></button>)}</div><aside className="notes-postscript" id="perspective"><span className="meta">Fighting content pollution</span><p>Something silly can have an exact purpose. Something beautifully produced can have none.</p></aside></section>
  <Detour/>
  <footer id="contact" className="contact-section"><div className="section-top"><span className="meta">A brief. A difficult question. A better explanation.</span></div><h2 className="contact-big"><span>Let’s talk.</span></h2><div className="contact-methods"><a href={linkedin} target="_blank" rel="noopener noreferrer"><span>LinkedIn</span><small>Connect with Varun</small><ArrowUpRight size={24}/></a><a href="mailto:varunpkashyap98@gmail.com"><span>Email</span><small>varunpkashyap98@gmail.com</small><ArrowUpRight size={24}/></a><a href="https://www.instagram.com/vruuun/" target="_blank" rel="noopener noreferrer"><span>Instagram</span><small>@vruuun</small><ArrowUpRight size={24}/></a></div><div className="footer-line"><span>Varun / Cultural insights & brand strategy</span><a href="#top">Back to the top ↑</a><span>Bangalore, India / © {new Date().getFullYear()}</span></div></footer>
 </main>
